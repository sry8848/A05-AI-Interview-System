...
public class InterviewSessionCache {
     ...
     */
//    为什么要用 Optional 包裹？
//    这是 Java 8+ 的标准做法，目的是优雅地处理“没查到”的情况。
//
//            Optional.of(session)：如果缓存里有这个会话，就把它装进 Optional 里。外部调用者拿到后可以放心地进行后续逻辑。
//
//            Optional.empty()：如果缓存里没查到（比如会话过期了或从未开始过），它不会返回 null 导致空指针异常，而是返回一个“空盒子”。
