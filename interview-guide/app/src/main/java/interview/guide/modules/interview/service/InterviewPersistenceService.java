...
//        Spring Data JPA 会根据解析方法名中的关键字，自动在后台拼接 SQL。我们来拆解 findFirstByResumeIdAndStatusInOrderByCreatedAtDesc：
//        findFirstBy: 对应 SQL 的 SELECT * ... LIMIT 1。意思是如果有多个符合条件的，只取最新的一条。
//        ResumeId: 对应 SQL 的 WHERE resume_id = ?。
//        AndStatusIn: 对应 SQL 的 AND status IN (?, ?, ...)。这解释了为什么代码里传了一个包含两种状态的 List。
//        OrderByCreatedAtDesc: 对应 SQL 的 ORDER BY created_at DESC。确保我们拿到的是用户最近一次开始的那场面试。
