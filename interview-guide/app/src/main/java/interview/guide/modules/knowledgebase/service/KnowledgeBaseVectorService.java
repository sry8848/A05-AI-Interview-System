...
public class KnowledgeBaseVectorService {
    ...
    public List<Document> similaritySearch(String query, List<Long> knowledgeBaseIds, int topK) {
        return similaritySearch(query, knowledgeBaseIds, topK, 0.0);
    }
    
    /**
     * 基于多个知识库进行相似度搜索（支持自定义相似度阈值）
     * 
     * @param query 查询文本
     * @param knowledgeBaseIds 知识库ID列表（如果为空则搜索所有）
     * @param topK 返回top K个结果
     * @param minScore 最小相似度阈值（0-1），0表示不过滤
     * @return 相关文档列表
     */
    ...
    try {
        ...
        SearchRequest.Builder builder = SearchRequest.builder()
            .query(query)                    // 设置查询文本
            .topK(Math.max(topK, 1));        // 确保至少返回1个结果
