...
//        为什么不用数据库自增 ID 作为会话 ID？”
//        你可以这样结合本项目回答：
//
//        安全性： 自增 ID（如 1, 2, 3）太容易被别人猜到。如果有人写个脚本遍历 ID，就能爬取别人的面试记录。UUID 这种随机字符串具有不可预测性。
//
//        解耦： 生成 SessionId 不需要先去数据库查询当前最大 ID 是多少，在内存里直接生成，性能更高，且能保证在分布式环境下的唯一性。
    ...
    public Optional<InterviewSessionDTO> findUnfinishedSession(Long resumeId) {
        try {
            ...
            if (cachedSessionIdOpt.isPresent()) {//如果找到了未完成的会话的会话ID
                String sessionId = cachedSessionIdOpt.get();
                Optional<CachedSession> cachedOpt = sessionCache.getSession(sessionId);//获取缓存的会话
                if (cachedOpt.isPresent()) {
    ...
    private InterviewSessionDTO toDTO(CachedSession session) {
        List<InterviewQuestionDTO> questions = session.getQuestions(objectMapper);//反序列化问题列表
        return new InterviewSessionDTO(
